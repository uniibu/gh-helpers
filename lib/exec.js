...
const exec = function(cmdLine, cli = false, npm = false) {
  return new Promise((resolve, reject) => {
    ...
    let env = process.env
    if (npm) {
      env = Object.assign({}, env, { npm_config_registry: 'https://registry.npmjs.org/' })
    }
    const pr = execa(parts.command, [].concat(parts.args), { input: stdin, shell: '/bin/bash', env: env, stdout, stderr })
