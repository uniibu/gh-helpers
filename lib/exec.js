...
const exec = function(cmdLine, cli = false, npm = false) {
  return new Promise((resolve, reject) => {
    ...
    let env = process.env;
    if (npm) {
      env = Object.assign({}, env, { npm_config_registry: 'https://registry.npmjs.org', npm_execpath: getNpmCli() })
    }
    console.log(env)
    const pr = execa(parts.command, [].concat(parts.args), { input: stdin, env: env, stdout, stderr })
